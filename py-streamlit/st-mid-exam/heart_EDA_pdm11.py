...
st.sidebar.subheader('Visualization using seaborn and plotly by pdm11')
...
with row1_1: 
    box_x = st.selectbox("Boxplot variable", options=df.columns, index=df.columns.get_loc("thal"))
...
with row2_2: 
    corr_y = st.selectbox("Correlation - Y variable", options=df.columns, index=df.columns.get_loc("sex"))
...
st.markdown('***')
st.header('df를 streamlit wepapp에 이용')
st.markdown('상관성/반상관성이 높은 변수들에 대한 자세한 시각화')

heatmap_data =df[["age","cp","thalach","exang","oldpeak","slope","ca","thal"]]
fig6 = plt.figure(figsize=(12,10))
sns.heatmap(heatmap_data.corr(), annot=True, vmin=-1, vmax=1, cmap='coolwarm')
st.pyplot(fig6)