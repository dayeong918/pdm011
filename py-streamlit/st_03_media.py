...
from PIL import Image #pillow 모듈에서 이미지 import
...
# ## Play audio file.
audio_file = open("files/loop_w_bass.mp3", "rb").read() #rb:read binary
st.audio(audio_file, format="audio/mp3", start_time=10)

st.header("Load local image from PC") #*딥러닝 
#### Load images
# Types of images
imgTypes = ["png", "jpg"]
 
st.info("Upload source image on Streamlit")
# # st.set_option('deprecation.showfileUploaderEncoding', False) *안해도 동작됨.
